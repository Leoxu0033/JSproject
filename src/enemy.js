...
export class Enemy {
  constructor(x, y, type = 'walker') {
    this.pos = { x, y };
    // Vector-based physics for steering behaviors
    this.posV = new Vector(x, y);
    this.velV = new Vector(0, 0);
    this.acc = new Vector(0, 0);
    this.maxSpeed = 4; // Default max speed (pixels per frame approx, or scaled)
    this.maxForce = 0.1; // Default max steering force
  ...
  initType(t) {
    ...
    if (this.type === 'tadpole') {
      ...
      // Boss State Machine
      this.bossState = 'track'; // track, windup, charge, recover
      this.bossTimer = 0;
  ...
  update(dt, game) {
    ...
    // behaviour per state
    if (this.type === 'drone') {
      // Sync posV from current pos
      this.posV.x = this.pos.x;
      this.posV.y = this.pos.y;
      
      const targetV = new Vector(player.pos.x + player.w/2, player.pos.y + player.h/2);
      
      this.seek(targetV);
      if (game.enemies) {
         this.separate(game.enemies);
      }
      
      this.velV.add(this.acc);
      this.velV.limit(this.maxSpeed);
      
      // Convert back to pixels/sec for the integrator
      this.vel.x = this.velV.x * 60;
      this.vel.y = this.velV.y * 60;
      ...
      // Constant spike emission
      if (this.spikeTimer > 0.6) {
          this.spikeTimer = 0;
          const count = 8;
          for (let i = 0; i < count; i++) {
              ...
              const spike = new Spike(this.pos.x + this.w/2 - 8, this.pos.y + this.h/2 - 8, vx, vy);
              game.entities.push(spike);
              game.enemies.push(spike);
          }
      }
      
      if (this.bossState === 'track') {
          // Smart pathfinding: avoid getting stuck under platforms when player is above
          ...
          if (player.pos.y < this.pos.y) {
              ...
              if (game.objects) {
                  for (const obj of game.objects) {
                      if ((obj.type === 'platform' || obj.type === 'oneway' || obj.type === 'wall') && rectsIntersect(probe, obj)) {
                          obstruction = obj;
                          break;
                      }
                  }
          ...
          if (obstruction) {
              ...
              const distToLeft = (this.pos.x + this.w/2) - obstruction.x;
              const distToRight = (obstruction.x + obstruction.w) - (this.pos.x + this.w/2);
              const escapeDir = distToLeft < distToRight ? -1 : 1;
              
              // Move horizontally to escape
              const escapeVel = new Vector(escapeDir * this.maxSpeed, 0);
              const steer = Vector.sub(escapeVel, this.velV);
          ...
          // Charge every 2.5 seconds
          if (this.bossTimer > 2.5) {
              this.bossState = 'windup';
              this.bossTimer = 0;
              this.velV.mult(0.2); // Slow down to prepare
          }
          this.velV.add(this.acc);
          this.velV.limit(this.maxSpeed);
          
      } else if (this.bossState === 'windup') {
          // Stop and shake (visuals handled in draw)
          this.velV.mult(0.9); // Decelerate
          if (this.bossTimer > 0.6) {
              this.bossState = 'charge';
              this.bossTimer = 0;
              // Launch towards player
              this.chargeDir = Vector.sub(targetV, this.posV).normalize();
              this.velV = Vector.mult(this.chargeDir, 18); // Very fast charge (approx 1080 px/s)
              if (game.audio) game.audio.playSfx('jump'); // Sound effect
          }
          
      } else if (this.bossState === 'charge') {
          // Move in straight line, no steering
          // Speed is set at launch, just maintain it or slight drag?
          // Let's keep it constant for impact
          if (this.bossTimer > 0.8) { // Charge duration
              this.bossState = 'recover';
              this.bossTimer = 0;
          }
          
      } else if (this.bossState === 'recover') {
          this.velV.mult(0.92); // Friction to stop
          if (this.bossTimer > 1.2) {
              this.bossState = 'track';
              this.bossTimer = 0;
          }
      }
    ...
    if (game.walls && game.walls.length && this.type !== 'boss') {
      const aheadRect = {
  ...
  this.pos.x += this.vel.x * dt;
    ...
    // Boss ignores wall collisions (does not die)
    if (game.objects && game.objects.length) {
      for (const obj of game.objects) {
        if (obj.type === 'slope') {
          ...
          if (eRight > obj.x && eLeft < obj.x + obj.w) {
            ...
            if (!isAboveSlope && eBottom >= slopeTop - 2 && eTop <= slopeTop + 5) {
              if (this.type === 'boss') {
                  // Boss bounces off slope (simple vertical push)
                  this.pos.y = slopeTop - this.h;
                  this.vel.y = 0;
                  this.posV.y = this.pos.y;
                  if (this.bossState === 'charge') {
                      this.bossState = 'recover';
                      this.bossTimer = 0;
                      if (game.screenShake) game.screenShake(10, 0.2);
                  }
              } else {
        ...
        if (this.type === 'tadpole') {
          ...
          if (eRight > obj.x && eLeft < obj.x + obj.w && eBottom > obj.y && eTop < obj.y + obj.h) {
            if (this.type === 'boss') {
                 // Resolve collision for Boss (AABB push out)
                 const overlapL = eRight - obj.x;
                 const overlapR = (obj.x + obj.w) - eLeft;
                 const overlapT = eBottom - obj.y;
                 const overlapB = (obj.y + obj.h) - eTop;
                 
                 const minOverlap = Math.min(overlapL, overlapR, overlapT, overlapB);
                 
                 if (minOverlap === overlapL) this.pos.x -= overlapL;
                 else if (minOverlap === overlapR) this.pos.x += overlapR;
                 else if (minOverlap === overlapT) this.pos.y -= overlapT;
                 else if (minOverlap === overlapB) this.pos.y += overlapB;
                 
                 // Sync posV
                 this.posV.x = this.pos.x;
                 this.posV.y = this.pos.y;
                 
                 // If charging, crash and stop
                 if (this.bossState === 'charge') {
                     this.bossState = 'recover';
                     this.bossTimer = 0;
                     if (game.screenShake) game.screenShake(12, 0.3);
                     if (game.spawnParticles) game.spawnParticles(this.pos.x + this.w/2, this.pos.y + this.h/2, '#fff', 10);
                 }
            } else {
  ...
  seek(target) {
    const desired = Vector.sub(target, this.posV);
    desired.normalize();
    desired.mult(this.maxSpeed);
    const steer = Vector.sub(desired, this.velV);
  ...
  separate(enemies) {
    const desiredSeparation = 35;
    const sum = new Vector(0, 0);
    let count = 0;
    for (const other of enemies) {
      if (other === this) continue;
      // Handle enemies that might not have updated posV
      const otherPos = other.posV || new Vector(other.pos.x, other.pos.y);
      const d = Vector.dist(this.posV, otherPos);
      if ((d > 0) && (d < desiredSeparation)) {
        const diff = Vector.sub(this.posV, otherPos);
        diff.normalize();
        diff.div(d);
        sum.add(diff);
        count++;
    ...
    if (count > 0) {
      ...
      const steer = Vector.sub(sum, this.velV);
      steer.limit(this.maxForce);
      this.applyForce(steer);
    }
  }
  ...
  draw(ctx) {
    ...
    if (this.type === 'tadpole') {
      ...
      // Drone: sleek triangle
      ctx.fillStyle = '#e0e0e0';
      const angle = this.velV.heading();
      ctx.rotate(angle);
      ctx.beginPath();
      ctx.moveTo(this.w/2, 0);
      ...
      ctx.beginPath();
      ctx.arc(-this.w/2, 0, 4, 0, Math.PI*2);
      ctx.fill();
    } else if (this.type === 'boss') {
      // Boss: Big menacing shape
      ctx.fillStyle = this.bossState === 'charge' ? '#ff0000' : (this.bossState === 'windup' ? '#ff5252' : '#d32f2f');
      
      // Shake effect during windup
      if (this.bossState === 'windup') {
          const shake = Math.sin(performance.now() / 20) * 3;
      ...
      for(let i=0; i<8; i++) {
        ...
        const sx = Math.cos(a) * (this.w/1.5);
        const sy = Math.sin(a) * (this.h/1.5);
        ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(0, 0, this.w/8, 0, Math.PI*2);
      ctx.fill();
    } else {
...
export class Spike {
  ...
  update(dt, game) {
    ...
    // Die on solid objects
    if (game.objects) {
        const bounds = this.getBounds();
        for (const obj of game.objects) {
            if (!rectsIntersect(bounds, obj)) continue;
            
            if (obj.type === 'slope') {
                // Precise slope collision
                const cx = this.pos.x + this.w/2;
                const cy = this.pos.y + this.h/2;
                if (cx >= obj.x && cx <= obj.x + obj.w) {
                    const localX = cx - obj.x;
                    const t = localX / obj.w;
                    const hAtX = obj.dir === 'right' ? (t * obj.h) : ((1 - t) * obj.h);
                    const slopeY = obj.y + (obj.h - hAtX);
                    if (cy > slopeY) {
                        this.alive = false;
                        if (game.spawnParticles) game.spawnParticles(this.pos.x + 8, this.pos.y + 8, '#ff0000', 3);
                        break;
                    }
                }
            } else {
  ...
  draw(ctx) {
    ctx.save();
    ctx.translate(this.pos.x + 8, this.pos.y + 8);
    ctx.rotate(performance.now() / 100);
    ctx.fillStyle = '#ff0000';
    ctx.beginPath();
    // Draw a star/spike shape
    for (let i = 0; i < 4; i++) {
